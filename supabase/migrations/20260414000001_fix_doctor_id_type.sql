-- ============================================================
-- FIX: Change p_doctor_id from UUID → TEXT in all RPCs
-- Doctor IDs are text strings like "doc1", "doc2", "doc3"
-- Run this in Supabase SQL Editor
-- ============================================================

-- 1. Drop old uuid-typed versions first
DROP FUNCTION IF EXISTS public.call_next_patient(uuid);
DROP FUNCTION IF EXISTS public.process_auto_no_shows();

-- 2. Recreate call_next_patient with TEXT parameter
CREATE OR REPLACE FUNCTION public.call_next_patient(p_doctor_id text)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- 3. Recreate process_auto_no_shows (calls updated function)
...
BEGIN
  ...
  LOOP
    ...
    -- Auto-advance the queue for that doctor (now accepts text)
